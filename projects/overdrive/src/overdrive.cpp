...
overdrive::overdrive(float setFs) :
...
void overdrive::deactivate() {}

void overdrive::update(int samples)
{
...
void overdrive::clear()
{
	prefilt.clear();
	postfilt.clear();
}

void overdrive::step(int len)
{
	//grab I/O pointers
	const float* pin = in;
	float* pout = out;
	...
	while (len > 0) {
		...
		waveshaper.stepBlock(inscr, pout, curlen);
