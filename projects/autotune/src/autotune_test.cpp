...
static void runSoundTest(bool automation)
{
	...
	//controls
	test.setControlValue(0, (int)scale::fundamental::F);		//fundamental
	test.setControlValue(1, (int)scale::mode::MAJOR);			//mode
	test.setControlValue(2, 430.0f);							//tuning
	test.setControlValue(3, 60);								//pitch1
	test.setControlValue(4, 50);								//pitch2
	test.setControlValue(5, 100.0f);								//smooth
	if (automation) {
	...
	testbuff inbuff(4800);
	testbuff outbuff(4800);
	inbuff.clear();
	outbuff.clear();
	...
	for (int it = 0; it < iter; ++it) {
		...
		inbuff.put(test.inputData(), bsize);

		test.step(bsize);

		plug.step(bsize);

		outbuff.put(test.outputData(), bsize);
...
static void runStressTest(bool automation)
{
	...
	test.setControlValue(1, (int)scale::mode::MAJOR);			//mode
	test.setControlValue(2, 430.0f);							//tuning
	test.setControlValue(3, 50);								//pitch1
	test.setControlValue(4, 81);								//pitch2
	test.setControlValue(5, 0.0f);								//smooth
	if (automation) {
...
int main()
{
	...
	//TEST SCALE LOGIC WITH SOME EXAMPLES
	/*
	scale S;
	S.set(scale::fundamental::Db, scale::mode::MAJOR, 440.0f);
