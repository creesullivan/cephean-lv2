...
static void runSoundTest(bool automation)
{
	...
	// Plugin setup -----------------------------
	doubleswitch plug(Fs);

	plugintester test(PLUG_CONTROLS, PLUG_INPUTS, PLUG_OUTPUTS, bsize);

	//controls
	test.setControlValue(0, 0.0f);		//switch1
	test.setControlValue(1, 1.0f);		//switch2
	if (automation) {
...
static void runStressTest(bool automation)
{
	...
	//controls
	test.setControlValue(0, 0.0f);		//switch1
	test.setControlValue(1, 0.0f);		//switch2
	if (automation) {
